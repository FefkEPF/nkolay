import { ArrowRight, Search, Palette, Server } from "lucide-react";
...
const ICONS: Record<string, React.ReactNode> = {
  performance: <Search className="w-6 h-6" />,
  creative: <Palette className="w-6 h-6" />,
  technical: <Server className="w-6 h-6" />,
};
          ...
          {SERVICE_CATEGORIES.map((category, i) => (
            ...
            >
              <div className="space-y-5">
                <div className="w-14 h-14 rounded-2xl bg-gray-50 flex items-center justify-center border border-gray-100 group-hover:scale-105 group-hover:bg-primary-light group-hover:border-primary/10 transition-all duration-500 ease-out text-gray-700 group-hover:text-primary">
                  {ICONS[category.id] ?? <Search className="w-6 h-6" />}
                </div>
                <div className="space-y-2">
                  <h3 className="font-display font-semibold text-xl text-gray-900 tracking-tight leading-snug group-hover:text-primary transition-colors duration-300">
                    {category.label}
                  </h3>
